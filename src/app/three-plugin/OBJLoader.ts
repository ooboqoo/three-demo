import { FileLoader, Material, LoadingManager, DefaultLoadingManager } from 'three';
...
export class OBJLoader {
  ...
  load(url: string, onLoad: (obj: THREE.Group) => any, onProgress?, onError?) {
  ...
  parse(text) {
    ...
    for (let i = 0, l = state.objects.length; i < l; i++) {
      ...
      if (createdMaterials.length > 1) {
        ...
        mesh = (!isLine ? new THREE.Mesh(buffergeometry, multiMaterial) : new THREE.LineSegments(
          buffergeometry, <any>multiMaterial));
      } else {
        mesh = (!isLine ? new THREE.Mesh(buffergeometry, createdMaterials[0])
