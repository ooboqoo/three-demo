...
export class C1Component implements AfterViewInit {
  ...
  private init() {
    ...
    Object.assign(this.plane.position, { x: 15, y: 0, z: 0 });
    ...
    Object.assign(this.cube.position, { x: -3, y: 3, z: 0 });
    this.cube.castShadow = true;
    this.scene.add(this.cube);

    const sphereGeometry = new SphereGeometry(4, 20, 20);
    const sphereMaterial = new MeshBasicMaterial({ color: 0x4444ff, wireframe: false });
    this.sphere = new Mesh(sphereGeometry, sphereMaterial);
    Object.assign(this.sphere.position, { x: 20, y: 4, z: 2 });
